import tkinter as tk

class OBSHitPointSyncForm:
    def __init__(self, root, submit_callback):
        self.root = root
        self.root.title("OBSHitPointSync")

        self.submit_callback = submit_callback

        tk.Label(root, text="OBS URL").grid(column=0, row=0, padx=10, pady=5)
        self.entry1 = tk.Entry(root, width=50)
        self.entry1.grid(row=0, column=1)
        self.entry1.insert(0, "ws://localhost:4444")

        tk.Label(root, text="Source Name").grid(row=1, column=0, padx=10, pady=5)
        self.entry2 = tk.Entry(root, width=50)
        self.entry2.grid(row=1, column=1)

        tk.Label(root, text="Filter Name").grid(row=2, column=0, padx=10, pady=5)
        self.entry3 = tk.Entry(root, width=50)
        self.entry3.grid(row=2, column=1)

        tk.Label(root, text="Character ID").grid(row=3, column=0, padx=10, pady=5)
        self.entry4 = tk.Entry(root, width=50)
        self.entry4.grid(row=3, column=1)

        submit_button = tk.Button(root, text="Submit", command=self.submit)
        submit_button.grid(row=4, column=0, columnspan=2, padx=10, pady=5)

    def submit(self):
        ws_url = self.entry1.get()
        sources = self.entry2.get()
        filter_name = self.entry3.get()
        character_id = self.entry4.get()
        self.submit_callback(ws_url, sources, filter_name, character_id)